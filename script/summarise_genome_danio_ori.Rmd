---
title: "A summary of the TE landscape of the original d_rario genome"
...
setwd('../../../')
WRK <- getwd()
SIM <- file.path(WRK, "TEgenomeSimulator_pub/output/TEgenomeSimulator_danio_result/")
OUT <- file.path(WRK, "TEgenomeSimulator_pub/output/sim_report/")
...
rmout <- rmout |> mutate(div = as.numeric(1 - identity), itg = as.numeric(length_ratio))  |>
  dplyr::mutate(superfamily = gsub("auto", "", superfamily)) |>
    ...
    dplyr::mutate(superfamily = gsub("LTR\\/Gypsy", "LTR\\/Ty3", superfamily)) |>
    dplyr::mutate(superfamily = gsub("DNA$", "DNA\\/unknown", superfamily)) |>
    dplyr::mutate(itg = if_else(itg > 1, 1, itg))
